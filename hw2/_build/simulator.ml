...
let step (m:mach) : unit =
  ...
  let ins_addr = regs.(rind Rip) in

  let instruction = List.hd @@ read (Imm (Lit ins_addr)) in
  let (opcode, opndList) = 
  ...
  let next = regs.(rind Rip) <- Int64.add ins_addr 8L in
