...
let rec lookup (x:string) (c:ctxt) : int64 =
  match c with 
    | (var_name, value)::cs  -> if var_name = x then value else lookup x cs
    | []            -> raise Not_found
...
let rec interpret (c:ctxt) (e:exp) : int64 =
  let rec aux = function 
    | (Var v)           -> lookup v c
    | (Const c)         -> c
    | (Add (e1, e2))    -> Int64.add (aux e1) (aux e2) 
    | (Mult (e1, e2))   -> Int64.mul (aux e1) (aux e2) 
    | (Neg e)           -> Int64.neg (aux e)
  in 
    aux e
...
let rec optimize (e:exp) : exp =
  ...
  | (Add (e1, e2))    -> 
    (match (optimize e1, optimize e2) with 
      ...
      | ((Neg (Const x)), (Const c))  -> Const (Int64.sub c x)
      | ((Const c), (Neg (Const x)))  -> Const (Int64.sub c x)
      | ((Const c), x)                -> if c = 0L then x else (Add ((Const c), x))
      | (x, (Const c))                -> if c = 0L then x else (Add (x, (Const c)))
      | (e1, e2)                      -> (Add (e1, e2))
    )
  | (Mult (e1, e2))   ->  
    (match (optimize e1, optimize e2) with 
      | (Const c1, Const c2)          -> (Const (Int64.mul c1 c2))
      | ((Neg (Const x)), (Const c))  -> Const (Int64.mul c (Int64.neg x))
      | ((Const c), (Neg (Const x)))  -> Const (Int64.mul c (Int64.neg x))
      | ((Const c), x)                -> if c = 0L then (Const 0L) else (Add (optimize e1, optimize e2))
      | (x, (Const c))                -> if c = 0L then (Const 0L) else (Add (optimize e1, optimize e2))
      | (e1, e2)                      -> (Mult (e1, e2))
    )
  | (Neg e)           -> 
...
let rec compile (e:exp) : program =
  ...
  let rec compile_opt (e:exp) : program =
    let rec aux ex p = 
      match ex with 
        | (Var v)           -> p @ [IPushV v]
        | (Const c)         -> p @ [IPushC c]
